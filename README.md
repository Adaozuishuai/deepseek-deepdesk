...
点击侧栏顶部的 **Agent** 切换到编码 Agent：

1. 选择工作目录（Agent 只在这个目录内读写文件）
2. 输入任务，如「看看这个项目有哪些文件」「写一个统计文件数的脚本并运行」
3. Agent 会自己读文件、写代码、执行命令，逐步完成
4. 完成任务会自动保存会话，点输入框的「历史」按钮随时回看

安全：三档权限模式（**每次询问 / 替我审批 / 完全访问**）决定命令与文件访问是否弹窗确认，可在 Agent 输入框左下角或 **设置 → 常规** 切换。
